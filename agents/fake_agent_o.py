...
TEAM_ID = os.getenv("TEAM_O_ID", "")
API_KEY = os.getenv("TEAM_O_API_KEY", "")
...
class FakeAgentO:
    ...
    def run(self) -> None:
        ...
        print(f"[O] Connected. Side={init_data.get('data', {}).get('side')}")

        while True:
            state = self.get_state()
            if state.get("status") != "success":
                print(f"[O] State error: {state.get('message')}")
            ...
            if status == "finished":
                print(f"[O] Match finished. Winner={data.get('winner')}")
                break
            ...
            if data.get("turn") == data.get("side"):
                ...
                print(f"[O] Move ({x}, {y}) => {result.get('status')} | {result.get('message')}")
...
if __name__ == "__main__":
    if not TEAM_ID or not API_KEY:
        print("Missing config. Set TEAM_O_ID and TEAM_O_API_KEY environment variables.")
        raise SystemExit(1)

    agent = FakeAgentO(SERVER_URL, TEAM_ID, API_KEY)
    try:
        agent.run()
    finally:
        agent._stop_hb.set()
