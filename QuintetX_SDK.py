"""QuintetX SDK (single-file)

Cách dùng (tối giản)
===================

1) Cài dependency (project này đã có sẵn `requests` trong Poetry). Nếu bạn chạy độc lập:

    pip install requests

2) Tạo client và connect (init):
    ...
    init = client.connect()            # POST /api/v1/agent/init
    print("side=", client.side)        # "X" hoặc "O" (server trả về)

3) Lấy state và gửi nước đi:

    state = client.get_state()         # GET /api/v1/agent/state
    if state["status"] == "success":
        data = state["data"]
        if data["match_status"] == "playing" and data["turn"] == data["side"]:
            client.send_move(20, 20)   # POST /api/v1/agent/move

4) Chạy vòng lặp tự động (poll state, đến lượt thì gọi strategy):

    def strategy(state_data: dict) -> tuple[int, int]:
        board = state_data["board"]
        # ví dụ: tìm ô trống đầu tiên
...
Ghi chú
------
- Auth của agent dùng headers: `X-API-Key` và `X-Team-ID`.
- SDK này dùng HTTP polling (đúng spec), không dùng WebSocket.
- Heartbeat: mặc định SDK sẽ chạy thread heartbeat sau khi connect.
